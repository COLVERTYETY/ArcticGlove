...
int Muca::getRegister(byte reg) {
  ...
  if (st != 0)  {
    Serial.println("[Muca] i2c write failed 1");
  }
...
void Muca::getRegisters(byte reg, byte size, byte * buffer) {
  ...
  unsigned int st = Wire.endTransmission(true);
  if (st != 0)  {
    Serial.println("[Muca] i2c write failed 2");
  }
...
bool Muca::init(bool interrupt) {
  ...
  Serial.println("[Muca] Interrupt set");
  setRegister(0xA7,0x04); // Set autocalibration
  Serial.println("[Muca] Autocalibration set");
}
...
// Read Data
  ...
  Wire.write(txAddr);
  unsigned int st = Wire.endTransmission(true);
  if (st != 0) Serial.println("[Muca] i2c write failed 3");
